...
package struct SearchMatch: Hashable, Codable {
    package let filePath: String
    package let lineNumber: Int // 0‑based
    package let lineText: String // original text (no newline)
    package let contextBefore: [String]? // Lines before the match (when contextLines > 0)
    package let contextAfter: [String]? // Lines after the match (when contextLines > 0)

    package init(filePath: String, lineNumber: Int, lineText: String, contextBefore: [String]? = nil, contextAfter: [String]? = nil) {
...
package struct SearchOptions {
    package var mode: SearchMode
    package var caseInsensitive: Bool
    package var wholeWord: Bool
    package var includeExtensions: [String] // e.g., [".js", ".ts", ".swift"]
    package var excludePatterns: [String] // e.g., ["node_modules", ".git", "*.log"]
    package var contextLines: Int // Number of lines before/after match
    package var maxResults: Int
    package var countOnly: Bool
    package var fuzzySpaceMatching: Bool // Enable/disable fuzzy space matching
    package var allowLiteralUnescapeFallback: Bool // Helpful rescue for over-escaped literals in auto flows
    package var contentFreshnessPolicy: FileContentFreshnessPolicy
    ...
    package init(
        ...
        self.fuzzySpaceMatching = fuzzySpaceMatching
        self.allowLiteralUnescapeFallback = allowLiteralUnescapeFallback
        self.contentFreshnessPolicy = contentFreshnessPolicy
    }
}

/// Codable wrapper for regex pattern errors
package struct PatternErrorInfo: Codable {
    package let errorType: String
    package let description: String

    package init(_ error: RegexPatternFailure) {
...
package struct PerFileError: Codable {
    package let filePath: String
    package let error: PatternErrorInfo

    package init(filePath: String, error: RegexPatternFailure) {
        self.filePath = filePath
...
package struct SearchResults: Codable {
...
package struct SearchFileDescriptor {
    package let id: UUID
    ...
    package let standardizedRootFolderPath: String
    package let fileExtension: String?
    package let contentSnapshot: (FileContentFreshnessPolicy) async throws -> FileSearchContentSnapshot
    ...
    package init(
        ...
        contentSnapshot: @escaping (FileContentFreshnessPolicy) async throws -> FileSearchContentSnapshot
    ) {
        self.id = id
        self.name = name
        ...
        self.standardizedRootFolderPath = standardizedRootFolderPath
        ...
        contentSnapshot = { policy in
            try await contentFetchCoordinator.withContentFetchPermit(for: store, searchID: searchID) {
                ...
                case .validateDiskMetadata:
                    let freshnessState = WorkspaceRuntimePerf.begin(
...
package actor FileSearchActor {
    ...
    private static func lineIndexCacheIdentity(
        ...
        return WorkspaceRuntimePerf.measure(
            WorkspaceRuntimePerf.Stage.Search.lineIndexCacheKey,
            WorkspaceRuntimePerf.Dimensions(fileBytes: content.utf8.count, scanKind: "hash-fallback")
        ) {
    ...
    private static func searchDocument(
        ...
        let lookupState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.lineIndexLookup,
            WorkspaceRuntimePerf.Dimensions(fileBytes: content.utf8.count, scanKind: cacheIdentity.scanKind)
        )
        ...
        WorkspaceRuntimePerf.end(
            WorkspaceRuntimePerf.Stage.Search.lineIndexLookup,
            lookupState,
            WorkspaceRuntimePerf.Dimensions(fileBytes: content.utf8.count, scanKind: cacheIdentity.scanKind, cacheHit: false)
        )

        let buildState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.lineIndexBuild,
            WorkspaceRuntimePerf.Dimensions(fileBytes: content.utf8.count, scanKind: cacheIdentity.scanKind)
        )
    ...
    private static func materializeMatches(
        ...
        return WorkspaceRuntimePerf.measure(
            WorkspaceRuntimePerf.Stage.Search.materializeMatches,
            WorkspaceRuntimePerf.Dimensions(
    ...
    private func searchContentWithErrors(
        ...
        let contentScanState = WorkspaceRuntimePerf.begin(
    ...
    private static func scanContentBatch(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.contentBatch,
            WorkspaceRuntimePerf.Dimensions(
    ...
    private static func scanFileWithErrorHandling(
        ...
        do {
            ...
            let snapshot = try await WorkspaceRuntimePerf.measure(
                WorkspaceRuntimePerf.Stage.Search.fileContentFetch,
                WorkspaceRuntimePerf.Dimensions(
                    scanKind: scanKind(for: plan),
                    isRegex: plan.engine != nil,
                    countOnly: plan.countOnly,
            ...
            if plan.countOnly {
                let fastPathState = WorkspaceRuntimePerf.begin(
                    WorkspaceRuntimePerf.Stage.Search.countOnlyFastPath,
                    WorkspaceRuntimePerf.Dimensions(
    ...
    private static func scanLiteral(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.literalScan,
            WorkspaceRuntimePerf.Dimensions(
    ...
    private static func scanPCRE2Regex(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.regexFullBufferScan,
            WorkspaceRuntimePerf.Dimensions(
                fileBytes: document.text.utf8.count,
                lineCount: document.lineRanges.count,
    ...
    private static func scanPCRE2RegexLineByLine(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.regexLineByLineScan,
            WorkspaceRuntimePerf.Dimensions(
                fileBytes: document.text.utf8.count,
                lineCount: document.lineRanges.count,
    ...
    private static func scanPathBatch(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.pathBatch,
            WorkspaceRuntimePerf.Dimensions(
    ...
    package func searchUnified(
        ...
        let perfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.actorSearchUnified,
            WorkspaceRuntimePerf.Dimensions(
        ...
        let resultConstructionState = WorkspaceRuntimePerf.begin(
    ...
    package static func inferredAutoMode(_ raw: String) -> SearchMode {
