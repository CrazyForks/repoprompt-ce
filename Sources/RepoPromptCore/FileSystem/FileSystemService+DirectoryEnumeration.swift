...
extension FileSystemService {
    ...
    package static func enumerateOneLevel(
        ...
        do {
            scan = try directoryListingBackend.listDirectoryWithIgnoreDetection(at: absFolder)
        } catch {
    ...
    package func scanFoldersInParallel(_ folders: Set<String>) async throws -> [FileSystemDelta] {
        ...
        // Capture actor-isolated dependencies before entering the task group.
        let fallbackRules = ignoreRules.snapshot()
        let directoryListingBackend = directoryListingBackend
                ...
                while inFlight < maxParallel, let folderRel = folderIterator.next() {
                    ...
                    group.addTask(priority: .utility) {
                        ...
                        try Self.enumerateOneLevel(
                            ...
                            directoryListingBackend: directoryListingBackend,
    ...
    package func scanOneLevelAndDiff(_ folderRelPath: String) async throws -> [FileSystemDelta] {
        ...
        let scanResult = try listDirectoryForCurrentFilesystem(absFolder)
    ...
    package static func buildDirectoryChunk(
        ...
        for entry in scanResult.entries {
            ...
            if isDirEntry {
                ...
                if trackCycles {
                    guard let id = service.directoryListingBackend.directoryIdentity(followingSymlinksAt: absolutePath) else {
                        continue
                    }
    ...
    package func walkPosixRecursivelyEmitChunks(
        ...
        let rootChain: DirChain?
            ...
            if isVirtualFS || skipSymlinks {
                rootChain = nil
            } else if let rootID = directoryListingBackend.directoryIdentity(followingSymlinksAt: rootFullPath) {
        ...
        static func processDirectoryOffActor(
            ...
            do {
                ...
                if testMode {
                    ...
                    scanResult = try Self.listDirectory(context.absPath, using: fm)
                } else {
                    scanResult = try service.directoryListingBackend.listDirectoryWithIgnoreDetection(at: context.absPath)
                }
            } catch {
    ...
    package func gatherPathsUsingEnumerator(
        ...
        let rootChain: DirChain?
            ...
            if isVirtualFS || skipSymlinks {
                ...
                var chain: DirChain?
                if let rootID = directoryListingBackend.directoryIdentity(followingSymlinksAt: canonicalRootPath) {
                ...
                if !baseRelativePath.isEmpty {
                    ...
                    for component in baseRelativePath.split(separator: "/") {
                        ...
                        if let id = directoryListingBackend.directoryIdentity(followingSymlinksAt: absPath) {
