...
/// tree projections.
package enum StoreBackedWorkspaceSearch {
    ...
    package static func search(
        ...
        readinessSource: (any WorkspaceSearchReadinessSource)?,
    ...
    package static func search(
        ...
        try await ensureSearchReady(store: store, readinessSource: readinessSource)
    ...
    package static func broadSearchAdmissionClass(
        pattern: String,
        mode: SearchMode,
        paths: [String]?
    ...
    private static func performSearch(
        ...
        let ingressFreshnessState = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.Search.ingressFreshnessWait)
        _ = await store.awaitAppliedIngress(rootScope: rootScope)
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.Search.ingressFreshnessWait, ingressFreshnessState)

        let entryPerfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.entrypoint,
        ...
        let scopePerfState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.Search.scopeFiltering,
            WorkspaceRuntimePerf.Dimensions(status: (paths?.isEmpty == false) ? "explicit" : "all", fileCount: allFiles.count)
        )
        ...
        if let rawPaths = paths, !rawPaths.isEmpty {
            ...
            if parsed.spec.clauses.isEmpty, let issue = parsed.issues.first {
                ...
                WorkspaceRuntimePerf.end(
                    WorkspaceRuntimePerf.Stage.Search.scopeFiltering,
                    scopePerfState,
                    WorkspaceRuntimePerf.Dimensions(status: "error", fileCount: allFiles.count)
                )
        ...
        WorkspaceRuntimePerf.end(
            WorkspaceRuntimePerf.Stage.Search.scopeFiltering,
            scopePerfState,
            WorkspaceRuntimePerf.Dimensions(status: (paths?.isEmpty == false) ? "explicit" : "all", fileCount: filesToSearch.count)
        )
        ...
        do {
            results = try await WorkspaceRuntimePerf.measure(
                WorkspaceRuntimePerf.Stage.Search.actorSearchCall,
                WorkspaceRuntimePerf.Dimensions(
                    searchMode: mode.rawValue,
    ...
    private static func ensureSearchReady(
        ...
        guard let readinessSource else { return }
        switch await readinessSource.readinessSnapshot().phase {
        case .ready, .degraded, .idle:
            return
