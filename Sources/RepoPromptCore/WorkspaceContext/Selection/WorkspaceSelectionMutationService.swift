...
package struct WorkspaceBuildSelectionResult: Equatable {
    package let selection: StoredSelection
    package let invalidPaths: [String]
    package let codemapUnavailable: [String]
}

package struct WorkspaceAddSelectionResult: Equatable {
    package let selection: StoredSelection
    package let invalidPaths: [String]
    package let resolvedMap: [String: String]
    package let mutated: Bool
    package let codemapUnavailable: [String]
}

package struct WorkspaceRemoveSelectionResult: Equatable {
    package let selection: StoredSelection
    package let invalidPaths: [String]
    package let resolvedMap: [String: String]
    package let mutated: Bool
...
package struct WorkspaceDemoteSelectionResult: Equatable {
    package let selection: StoredSelection
    package let invalidPaths: [String]
    package let codemapUnavailable: [String]
    package let mutated: Bool
...
package struct WorkspaceSliceSelectionMutationResult: Equatable {
    package let selection: StoredSelection
    package let invalidPaths: [String]
    package let resolvedMap: [String: String]
    package let mutated: Bool
...
package struct WorkspaceSelectionMutationService {
    package let store: WorkspaceFileContextStore
    package let codemapsGloballyDisabled: Bool
    package let codemapsGloballyDisabledMessage: String
    ...
    package func addPaths(
        ...
        let candidateResolutionTotal = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.candidateResolutionTotal)
        ...
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.candidateResolutionTotal, candidateResolutionTotal)

        let structuralMerge = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.structuralMerge)
        ...
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.structuralMerge, structuralMerge)
        let autoCodemapRecomputeTotal = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.autoCodemapRecomputeTotal)
        if selection.codemapAutoEnabled {
            selection = await recomputeAutoCodemaps(selection, rootScope: rootScope)
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.autoCodemapRecomputeTotal,
                autoCodemapRecomputeTotal,
                WorkspaceRuntimePerf.Dimensions(outcome: "attempted")
            )
        } else {
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.autoCodemapRecomputeTotal,
                autoCodemapRecomputeTotal,
                WorkspaceRuntimePerf.Dimensions(outcome: "skipped")
            )
        }
    ...
    package func resolveSelectionCandidates(
        paths: [String],
        rawPaths: [String],
        expandFolders: Bool,
    ...
    package func recomputeAutoCodemaps(
        ...
        let selectedFileLookup = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.selectedFileLookup)
        let resolved = await store.lookupFiles(atPaths: base.selectedPaths, rootScope: rootScope)
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.selectedFileLookup, selectedFileLookup)
        ...
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.codemapAPILoad, codemapAPILoad)
        guard !aggregate.orderedFileAPIs.isEmpty else {
            return StoredSelection(selectedPaths: base.selectedPaths, autoCodemapPaths: [], slices: base.slices, codemapAutoEnabled: base.codemapAutoEnabled)
        }
        let referencedPathResolution = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.referencedPathResolution)
        ...
        WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.ReadFile.AutoSelect.referencedPathResolution, referencedPathResolution)
