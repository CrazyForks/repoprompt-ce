...
private actor ContentReadAsyncLimiter {
    ...
    private func enqueueWaiter(
        ...
        WorkspaceRuntimePerf.lifecycleEvent(
            WorkspaceRuntimePerf.Lifecycle.FileSystem.contentReadWorkerPermitWaitBegan,
    ...
    private func cancelWaiter(id: UUID) {
        if case let .waiting(continuation, workloadClass, lifecycleCorrelation)? = waiterStates.removeValue(forKey: id) {
            ...
            WorkspaceRuntimePerf.lifecycleEvent(
                WorkspaceRuntimePerf.Lifecycle.FileSystem.contentReadWorkerPermitCancelled,
                correlation: lifecycleCorrelation,
                WorkspaceRuntimePerf.Dimensions(
                    workloadClass: workloadClass.rawValue,
                    queueDepth: waiterOrder.count,
    ...
    private func release() {
        while !waiterOrder.isEmpty {
            ...
            case let .waiting(continuation, workloadClass, lifecycleCorrelation):
                WorkspaceRuntimePerf.lifecycleEvent(
                    WorkspaceRuntimePerf.Lifecycle.FileSystem.contentReadWorkerPermitAcquired,
                    correlation: lifecycleCorrelation,
                    WorkspaceRuntimePerf.Dimensions(
                        workloadClass: workloadClass.rawValue,
                        queueDepth: waiterOrder.count,
...
extension FileSystemService {
    ...
    package func loadContent(
        ...
        WorkspaceRuntimePerf.lifecycleEvent(
            ...
            correlation: lifecycleCorrelation,
            WorkspaceRuntimePerf.Dimensions(workloadClass: workloadClass.rawValue, rootToken: diagnosticRootToken.uuidString)
        )
        let contentLoadState = WorkspaceRuntimePerf.begin(
        ...
        let preparationState = WorkspaceRuntimePerf.begin(
        ...
        do {
            ...
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.FileSystem.contentReadRequestPreparation,
                preparationState,
                WorkspaceRuntimePerf.Dimensions(outcome: "prepared", workloadClass: workloadClass.rawValue)
            )
        } catch {
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.FileSystem.contentReadRequestPreparation,
                preparationState,
                WorkspaceRuntimePerf.Dimensions(outcome: "error", workloadClass: workloadClass.rawValue)
            )
            throw error
        }
        WorkspaceRuntimePerf.lifecycleEvent(
    ...
    package func loadContentWithDate(
        ofRelativePath relativePath: String,
        workloadClass: ContentReadWorkloadClass = .unspecified
    ...
    package func loadEntireFileContentOptimized(
        ofRelativePath relativePath: String,
    ...
    private func makeContentReadRequest(
        ...
        let contentLoadState = WorkspaceRuntimePerf.begin(WorkspaceRuntimePerf.Stage.FileSystem.contentLoadActorBody)
        defer { WorkspaceRuntimePerf.end(WorkspaceRuntimePerf.Stage.FileSystem.contentLoadActorBody, contentLoadState) }
    ...
    private func performMeasuredContentReadOffActor(
        ...
        WorkspaceRuntimePerf.lifecycleEvent(
            ...
            correlation: lifecycleCorrelation,
            WorkspaceRuntimePerf.Dimensions(workloadClass: request.workloadClass.rawValue, rootToken: diagnosticRootToken.uuidString)
        )
        let offActorState = WorkspaceRuntimePerf.begin(
        ...
        do {
            ...
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.FileSystem.contentReadOffActorAwait,
                offActorState,
                WorkspaceRuntimePerf.Dimensions(outcome: result.telemetryOutcome.rawValue, workloadClass: request.workloadClass.rawValue)
            )
            WorkspaceRuntimePerf.lifecycleEvent(
            ...
            WorkspaceRuntimePerf.end(
                WorkspaceRuntimePerf.Stage.FileSystem.contentReadOffActorAwait,
                offActorState,
                WorkspaceRuntimePerf.Dimensions(outcome: outcome, workloadClass: request.workloadClass.rawValue)
            )
            WorkspaceRuntimePerf.lifecycleEvent(
                WorkspaceRuntimePerf.Lifecycle.FileSystem.contentReadWorkerReturned,
                correlation: lifecycleCorrelation,
    ...
    private nonisolated static func readContentFromDisk(_ request: ContentReadRequest) async throws -> ContentReadResult {
        let workerBodyState = WorkspaceRuntimePerf.begin(
            WorkspaceRuntimePerf.Stage.FileSystem.contentReadWorkerBody,
            WorkspaceRuntimePerf.Dimensions(
                workloadClass: request.workloadClass.rawValue,
                contentSource: "disk"
            )
    ...
    package func detectEncodingForInitialChunk(initialData: Data) throws -> String.Encoding {
    ...
    package func detectFileEncodingFromData(_ data: Data) async throws -> String.Encoding {
        // 1) BOM check
    ...
    package static func isProbablyBinary(_ data: Data, sampleSize: Int = 8192) -> Bool {
    ...
    package static let orderedFallbackEncodings: [String.Encoding] = [
        .windowsCP1252,
        .macOSRoman
    ]

    /// Optional, low‑priority locale‑specific single‑byte encodings
    package static let regionSpecificEncodings: [String.Encoding] = [
    ...
    package static let alwaysBinaryExtensions: Set<String> = [
        // ── Video ───────────────────────────────────────────────────
    ...
    package static let alwaysTextExtensions: Set<String> = [
        // ── General text / docs ─────────────────────────────────────
    ...
    package static let alwaysTextFilenames: Set<String> = [
    ...
    package static func detectBOMEncoding(in data: Data) -> String.Encoding? {
    ...
    package func readDataAndDetectEncoding(_ fullPath: String) throws -> DetectedText {
    ...
    package static func looksLikeUTF16(_ data: Data) -> Bool {
    ...
    package func detectFileEncoding(atRelativePath relativePath: String) async throws -> String.Encoding {
