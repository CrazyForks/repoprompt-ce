...
package enum LanguageTypeExtractor {
    // MARK: - Swift Patterns
    ...
    package static let swiftVariableRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^
    (?:[-*]\s*)?
    \s*
    ...
    package static let cSharpVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    package static let cSharpFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:public|private|protected|internal)?\s*
    ...
    package static let javaVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    package static let javaFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:public|private|protected)?\s*
    ...
    package static let dartVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:
     (?:final|const)\s+
    ...
    package static let dartFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    (?:
    ...
    package static let dartFactoryRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    factory\s+
    ...
    package static let dartGetterSetterRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    ...
    package static let cVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    package static let cFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:extern|static)?\s*
    ...
    package static let cppVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    package static let cppFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:template\s*<[^>]*>\s*)?
    ...
    package static let cppConstructorRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^\s*
    (?:template\s*<[^>]*>\s*)?
    ...
    package static let cppTrailingReturnFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:template\s*<[^>]*>\s*)?
    (?:inline\s+|static\s+|virtual\s+|constexpr\s+|const\s+|unsigned\s+|signed\s+|long\s+|short\s+|typename\s+|friend\s+)*
    ...
    package static let pythonVariableRegex: Regex<(Substring, Substring, Substring)> =
    ...
    package static let pythonFunctionRegex: Regex<(Substring, Substring, Substring, Substring?)> =
    ...
    /// Output: (wholeMatch, varName, typeName?)
    package static let jsTsVariableRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:var|let|const)\s+([A-Za-z_]\w*\??)\s*(?::\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?"#)

    /// JS/TS function: `(export)? (default)? (async)? function name<T>?(params): ReturnType?`
    /// Output: (wholeMatch, funcName, paramList, returnType?)
    package static let jsTsFunctionRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?function\s+([A-Za-z_]\w*\??)(?:<[^>]+>)?\s*\(([^)]*)\)(?:\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?(?:\s*\{)?"#)

    /// TS arrow function pattern
    package static let tsArrowFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    ...
    /// Output: (wholeMatch, funcName, paramList, returnType)
    package static let tsArrowFunctionParamsReturnRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:var|let|const)\s+([A-Za-z_]\w*\??)\s*=\s*\(([^)]*)\)\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-\s,]*)\s*=>"#)

    /// TS type alias: `(export)? type Name = Type`
    /// Output: (wholeMatch, aliasName, rhsType)
    package static let tsTypeAliasRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?type\s+([A-Za-z_]\w*)\s*=\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*)"#)

    /// TS variable: `(export)? (var|let|const) name?: Type`
    /// Output: (wholeMatch, varName, typeName?)
    package static let tsVariableRegex = CodeMapPCRE2Pattern(#"^[ \t]*(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:var|let|const)\s+([A-Za-z_]\w*\??)(?:\s*:\s*([A-Za-z_][A-Za-z0-9_<>\|\[\]\(\)\{\}\&\?\.\-]*))?"#)

    // MARK: - TSX

    /// Very similar to TS but we allow a generic in the name, e.g. "function FooComponent<T>()"
    package static let tsxVariableRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    (?:var|let|const)\s+
    ...
    package static let tsxFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    function\s+
    ...
    package static let tsxArrowFunctionRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?(?:async\s+)?
    (?:var|let|const)\s+
    ...
    package static let tsClassMethodRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    package static let tsClassPropertyRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    package static let tsClassArrowMethodRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    package static let tsClassArrowNoParensRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    (?:(?:public|private|protected|static|readonly|abstract|async)\s+)*
    ...
    package static let tsAccessorRegex = CodeMapPCRE2Pattern(#"""
    (?xm)
    ^[ \t]*
    (?:export\s+)?(?:default\s+)?
    ...
    package static let goVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    (?:var|const)\s+
    ([A-Za-z_]\w*)
    ...
    package static let goFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    package func\s+
    (?:\([^)]*\)\s+)?        # optional receiver
    ...
    package static let rustVariableRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    package let\s+(?:mut\s+)?
    ([A-Za-z_]\w*)
    ...
    package static let rustFunctionRegex: Regex<AnyRegexOutput> = try! Regex(#"""
    (?xm)
    ^
    ...
    package static func matchAnyVariableLine(
        _ line: String,
    ...
    package struct FunctionLineMatch {
        let name: String?
    ...
    package static func matchAnyFunctionLineParsed(
        _ line: String,
        language: LanguageType,
        stats: CodeMapPerfStats? = nil
    ) -> FunctionLineMatch? {
    ...
    package static func matchAnyFunctionLine(
        _ line: String,
        language: LanguageType,
        stats: CodeMapPerfStats? = nil
    ) -> [String: String]? {
    ...
    package typealias RegexMatch = Regex<AnyRegexOutput>.Match
    ...
    package static func extractNamedGroups(
        match: RegexMatch,
