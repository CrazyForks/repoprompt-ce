...
package struct SliceAnchor: Codable, Equatable {
    package var range: LineRange
    package var startSignature: [String]
    package var endSignature: [String]
...
package typealias PartitionStoreSaveEventSink = @Sendable (PartitionStoreSaveEvent) -> Void

package actor PartitionStore {
    ...
    package struct StoredSlices: Codable, Equatable {
        package var ranges: [LineRange]
        package var fileModificationTime: Double?
        package var anchors: [SliceAnchor]?
    ...
    package struct PartitionData: Codable {
        var version: Int
    ...
    private let saveEventSink: PartitionStoreSaveEventSink
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder
    private let dateFormatter = ISO8601DateFormatter()

    package init(baseURL: URL, saveEventSink: @escaping PartitionStoreSaveEventSink = { _ in }) {
        self.baseURL = baseURL.standardizedFileURL
        self.saveEventSink = saveEventSink
    ...
    package func load(forRoot rootPath: String, scope: PartitionScope) async -> PartitionData {
    ...
    package func save(forRoot rootPath: String, scope: PartitionScope, data: PartitionData) async throws {
        ...
        saveEventSink(PartitionStoreSaveEvent(
            rootPath: (rootPath as NSString).standardizingPath,
            scope: scope,
    ...
    package func load(forRoot rootPath: String, workspaceID: UUID) async -> PartitionData {
        await load(forRoot: rootPath, scope: PartitionScope(workspaceID: workspaceID))
    }

    package func save(forRoot rootPath: String, workspaceID: UUID, data: PartitionData) async throws {
