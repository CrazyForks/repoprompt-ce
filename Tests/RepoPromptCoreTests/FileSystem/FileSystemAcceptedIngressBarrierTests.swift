...
final class FileSystemAcceptedIngressBarrierTests: XCTestCase {
    ...
    func testMailboxStaleDrainCompletionDoesNotClearRestartDrain() async {
        ...
        _ = mailbox.accept(firstPayload, acceptanceGeneration: oldAcceptanceGeneration, diagnosticContext: nil) {
        ...
        _ = mailbox.stopAccepting()
        mailbox.discardPendingAndCancelDrain()
        let newAcceptanceGeneration = mailbox.startAccepting()

        _ = mailbox.accept(secondPayload, acceptanceGeneration: newAcceptanceGeneration, diagnosticContext: nil) {
